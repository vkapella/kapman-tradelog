...
| 2 | Upload fixture `History_for_Account_X19467537-8.csv` on `/imports` with adapter `fidelity` | Preview loads and displays row count and status badge breakdown |
...
| 7 | Re-upload and commit fixture `History_for_Account_X19467537-8.csv` | `inserted.executions = 0` and `skippedDuplicates.executions` equals first commit execution inserts |
| 8 | Upload fixture `History_for_Account_X19467537-9.csv` | Assignment rows (DAL, INTC) import without errors and both option-close and equity-delivery rows are present |
