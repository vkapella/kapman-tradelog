...
| Engine cash | `AccountValueSnapshot.cashValue` = account `startingCapital` + classified cash-event amounts + per-execution trade cash. Trade cash uses the broker's settled `netAmount` for Fidelity rows, so commissions and regulatory fees reach cash (#372); other brokers use quantity × price × multiplier because thinkorswim's `netAmount` is a per-unit net price. A money-market or securities position held before the first exported row never appears in the export, so `startingCapital` must carry the cash-equivalents held on the day before history begins. |
