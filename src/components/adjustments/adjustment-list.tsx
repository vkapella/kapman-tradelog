...
const AdjustmentRow = memo(function AdjustmentRow({ record, onReverse, reversingId, supersededIds }: { record: ManualAdjustmentRecord; onReverse: (id: string) => void; reversingId: string | null; supersededIds: Set<string>; }) {
  return (
    <>
      ...
      <div className="px-2 py-2"><span className={record.status === "ACTIVE" ? "text-pos" : "text-text-2"}>{record.status}</span>{record.status === "ACTIVE" && supersededIds.has(record.id) ? <span className="ml-1 text-[10px] text-warn">(SUPERSEDED)</span> : null}</div>
